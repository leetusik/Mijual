...
doc_id: qa
version: v0010
created_at: 2026-08-31T04:33:50+09:00
source: P10.REVIEW
summary: P10 round 2 — the rebrand block is corrected for the second mark and the shipped favicon, and six R17 lines are added
previous: v0009_p10_rebrand_five_checklist_lines_guard_the_painted_mark_both_titles_the_old-name_sweep_including_the_live_agent_the_retired_binaries_and_the_still-absent_favicon
---
...
- [ ] `/ask` 빈 상태 = 시작 화면: **질문 카드 4장** (2 columns, 1 at ≤767, pressing one sends its sentence verbatim) · 익명 줄 0 · 새 대화 0 · 「범위:」 칩 0 · composer with the input's own 1px only (no double frame, no divider) (P9)
...
- [ ] 인용 칩 (both places): a quoted chip opens quote + DART 원문 in place (180px cap, re-tap closes, `inert` when shut); a span-less chip opens the DART link **alone** with no explanatory sentence; in a 데이터 행 / 계산 입력 the chip holds the fixed third column and the panel opens **under the row across the block** (P9)
...
- [ ] **The brand mark is actually painted, not just referenced.** On every reader route (landing,
      ...
      `complete && naturalWidth > 0` at **94.02×27** and **83.58×24**, natural **1292×371**,
      `transform: translateY(-7px)` / `-6px`, `alt="주주의관제탑"` — **at 1280 and at 390 alike**,
      because R17 places the mark with no viewport branch. No type and no test catches a wrong asset
      path — `tsc` was clean over a 404 URL — so this is asserted in a browser or not at all
      (P10, corrected P10 round 2)
...
- [ ] **The favicon is served**: `document.querySelectorAll('link[rel*="icon"]').length === 3` on
      ...
      hand-written `<link>`. Each tile is **opaque** (no alpha channel at all — transparent vanishes
      on a light tab) on `#0a1310`, ink width **84 %** of the box, centred on both axes. `favicon.ico`
      is deliberately absent. *(This line replaces P10 round 1's "still no favicon" guard, which
      guarded the absence until the operator decided — they decided.)* (P10 round 2)
...
- [ ] **크롬 마크**: the 주주의관제탑 wordmark paints in nav and footer at **h27 / h24** with its glyph
      band on the host's optical centre — nav band centre **25.60** against a **51px** content box
      (the bar is 52px *border-box* with a 1px rule), footer **+0.31px**, minimum vertical clearance
      **5.0px**. A 0.5px difference from R17's own absolute figures is **expected**, not drift (P10)
