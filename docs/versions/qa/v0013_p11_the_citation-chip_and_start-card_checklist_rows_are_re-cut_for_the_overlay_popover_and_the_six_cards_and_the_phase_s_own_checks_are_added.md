...
doc_id: qa
version: v0013
created_at: 2026-08-31T16:14:05+09:00
source: P11.REVIEW
summary: P11: the citation-chip and start-card checklist rows are re-cut for the overlay popover and the six cards, and the phase's own checks are added
previous: v0012_p10_round_4_the_falsified_vertical-geometry_line_is_replaced_by_a_check_that_can_fail_the_mark_s_glyph_band_reads_on_one_line_with_the_hangul_beside_it_0.5px_with_its_negative_control_and_its_n_a-at-390_construction
---
...
- [ ] `/ask` 빈 상태 = 시작 화면: **질문 카드 6장** (2 columns → three even rows with no orphan, 1 column at ≤767, pressing one sends its sentence verbatim) · 익명 줄 0 · 새 대화 0 · 「범위:」 칩 0 · composer with the input's own 1px only (no double frame, no divider). **And the capability claim the six exist for:** pressing them one at a time yields **six distinct 도구 행** — 이벤트 검색 · 이벤트 읽기 (by 접수번호, **no** 검색 row before it) · 계산 · 내 포트폴리오 읽기 · 의견 저장 · 운영자 연락처 — every answer real, **no 「미확인」 anywhere**, and no invented contact when `MIJUAL_OPERATOR_CONTACT` is unset. *Pressing the 의견 card writes a real `conversation_feedback` row every time; that is the capability, not a bug — count the rows before and after a verification pass* (P9, corrected P11)
...
**P11 block — the ask surface's inline citations and the six capability cards** (added at the P11
review; every box re-run in the Operator Runtime — `make stack-up`, `next dev` on `127.0.0.1:3010`
**and** `npm run build && npm run start` on the same origin — in real Chrome 152 over CDP at 1280 and
at a true 390 device-metrics emulation. *Aside is the workspace's preferred instrument and is not
installed on this machine; the fallback applies — same sweep, same viewports, same runtime, real
browser*):

- [ ] **한 문장이 근거 2건 이상을 지고도 한 줄로 읽힌다**: ask 「HLB 전환사채 공시가 몇 건이나 있나요?」
      and the answer's sentence carries **four chips side by side after its period** — same `y` to the
      decimal, 17px apart, `<p class=prose>` **one line high** (21px at 20.925px leading) and **0
      `<br>`**. Measured 1280 `[530.1, 547.1, 564.1, 581.1] @ y 249` and 390 `[286.1 … 337.1] @ y 261`,
      identical in dev and production, on `/ask` **and** in the widget. **The teeth:** this is the
      defect the phase existed to close (`…입니다.[1] ⏎ [2] ⏎ [3]`) — a paragraph taller than one line
      for a one-line answer, or `.sentence + .sentence`'s computed `margin-left` back at `0px` instead
      of `3.375px`, is the regression returning (P11)
- [ ] **칩을 열어도 아무것도 움직이지 않는다**: a 40-element document-coordinate snapshot (`.prose`,
      every `.sentence`, every `.row`, `.rowLabel`, `.rowValue`, `.answer`, `.data`, `.calc`, every
      chip) is **byte-identical before and after the click** in all three placements, the data row's
      grid tracks stay `283.188px 391.812px 17px` (**the value column never collapses**), and in the
      widget the thread's `scrollHeight` is **unchanged** (509 → 509) so the composer is not pushed.
      One popover at a time; 칩 재탭, a press outside and **Esc (focus back on the chip)** all close it
      (P11)
- [ ] **팝오버의 자리와 바탕**: 프로즈 → 380px under the chip (340px at ≤767, slid back fully inside the
      viewport); 데이터 행 값 / 계산 입력 → anchored to `.row`, **block-wide** (732px at 1280, 354px at
      390), flush under the row; inside the widget it stays **inside the thread and inside the widget**
      (clamped by `nearestClip`, flipping up when the thread would cut it). Ground is the product's
      overlay ground `rgb(14, 26, 21)` + `border-left: 2px rgb(95, 208, 165)` + `z-index: 40`, quote
      `max-height: 180px` — **opaque is deliberate** (R16 signed `--surface-inset` for an in-flow
      panel); asserting the translucent token now fails on a correct build (P11)
