...
doc_id: qa
version: v0003
created_at: 2026-08-22T18:21:11+09:00
source: P5.REVIEW
summary: P5 apply phase: the suite at 118 plus the framework-free frontend check, real-browser verification as a method, and D1/D4 closed off the fragile list
previous: v0002_extraction-accuracy_measurement_method_the_frozen_evalset_cross-model_provenance_and_the_first_measured_numbers
---
...
The Python suite grew **59 → 118** across the phase, and it still runs in ~2.6 s with no network, no
...
|---|---|---|
| unit suite | `.venv/bin/python -m pytest` | **118 passed**, ~2.6 s, no network, no model, no DB (one known Starlette/httpx deprecation warning) |
| workspace | `python3 scripts/workflow.py validate` | `Workflow validation passed.` |
| **frontend** | `cd frontend && npm run build && npm run typecheck && npm run smoke` | build green (**16 routes**), `tsc --noEmit` clean, **11/11** `node:test` cases in ~90 ms |
...
- [ ] `pytest` green (**118**) and `workflow validate` clean.
- [ ] `cd frontend && npm run build && npm run typecheck && npm run smoke` green (**11/11**).
- [ ] `gates run` twice → **byte-identical** output, and the verdict split unchanged over **710** rows
      unless the corpus grew.
- [ ] **The structural guards still guard**: the AST import scans (no request-path or derivation-layer
      module imports a spending module; only the vocky module imports an HTTP client), the ops surface
      carrying **no** unsafe method beyond login/logout, and `WriteSession` refusing a safe method.
      These encode rules that are cheap to break by accident and expensive to notice.
