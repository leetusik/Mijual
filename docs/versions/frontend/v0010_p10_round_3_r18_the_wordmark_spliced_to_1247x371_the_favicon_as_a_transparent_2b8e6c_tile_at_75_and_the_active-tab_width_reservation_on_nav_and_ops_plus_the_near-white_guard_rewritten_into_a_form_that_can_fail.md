...
doc_id: frontend
version: v0010
created_at: 2026-08-31T11:10:41+09:00
source: P10.REVIEW
summary: P10 round 3 (R18): the wordmark spliced to 1247x371, the favicon as a transparent #2b8e6c tile at 75%, and the active-tab width reservation on nav and /ops — plus the near-white guard rewritten into a form that can fail
previous: v0009_p10_round_2_r17_ships_nav_h27_footer_h24_with_the_ink_offset_in_the_component_a_32px_mask-painted_launcher_with_zero_animation_the_footer_corner_reservation_the_favicon_at_16_32_180_and_a_self-hosted_noto_sans_kr_subset
---
...
| R2 §Page shell wordmark **h19 / h17**, box-centred | **R17** | **h27 / h24**, ink-aligned `translateY(-7px)` / `-6px` — one placement at every viewport, no breakpoint |
...
**The heights are `h27` (nav) / `h24` (footer), and the placement is ink-aligned, not
box-centred** (R17, superseding R2's h19/h17). The `<img>` computes to **90.75×27** and **80.66×24**
(R18's narrower box; the arithmetic is `1247/371 = 3.3612`, and Chrome quantises the footer width to
80.66), dev and a production build agreeing to the hundredth of a pixel, at **every** viewport — there is no
breakpoint on the mark. **The offset is a module constant inside `Wordmark.tsx`
(`{27: 7, 24: 6}` → `translateY(-7px)` / `-6px`), not a call-site prop**, so no consumer can forget
it. It exists because the mark's ink is bottom-half only: the Korean glyph band is the bottom
**47.44 %** of the box with the sparkle cluster flush to the top-right and 30 empty rows between, so
centring the *box* sits the *name* low.

Measured in the running product: the nav bar is `height: 52px` **border-box** with a 1px bottom rule,
so the content box `.inner` stretches into is **51px** — the mark's box top lands at **5.00px**, its
bottom at 32.00, bottom clearance **19.00px**, and the glyph band's centre at **25.60px** against the
bar's real optical centre of **25.50px**: **+0.1px**. (R17's own absolute figures — 5.5 / 26.10 /
19.5 — are each exactly 0.5px high because it computed from a 52px *content* box; the relationship it
signed holds to 0.1px, which is why the numbers were corrected at apply time and the record was not
edited.) In the footer the band centre lands **+0.31px** off its row's optical centre, which is
entirely the integer-px rounding R17 itself mandates (`0.2628 × 24 = 6.307` ships as `-6`).
