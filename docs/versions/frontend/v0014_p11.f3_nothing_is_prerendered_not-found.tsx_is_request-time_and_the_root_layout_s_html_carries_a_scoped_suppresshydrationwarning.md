...
doc_id: frontend
version: v0014
created_at: 2026-09-01T03:26:53+09:00
source: P11.REVIEW
summary: P11.F3: nothing is prerendered — not-found.tsx is request-time, and the root layout's <html> carries a scoped suppressHydrationWarning
previous: v0013_p11_fixes_the_ask_route_becomes_the_start_screen_s_data_boundary_with_a_per-card_static_fallback_and_the_global_footer_publishes_served_config_read_by_the_root_layout
---
...
- **주주의관제탑 owns its 404.** `app/not-found.tsx` + `RequestedPath.tsx` render the Korean not-found for
  ...
  Since `P11.F3` the page is **request-time**, so the echo is the reader's own address on the first
  paint. Two things measured about it at `P11.REVIEW` and left as they are, because both are
  decisions rather than defects: the echo is the **percent-encoded** pathname (`usePathname()`'s own
  value), so a Korean URL renders as `/%EC%96%B4…` on an otherwise Korean-only screen; and this is
  the one route that carries **no** `link[rel=preload][as=font]` (both faces still load and paint,
  one hop later). A `notFound()` thrown from a **dynamic** segment still returns Next's
  `<html id="__next_error__">` shell with no SSR content and draws the whole screen on the client —
  no hydration, so no mismatch, but an empty first paint (**D35**).
...
- **Rendering:** per surface, and every data-bearing route is **request-time** (`connection()`,
  ...
  must be live. **Since `P11.F3` no HTML route is prerendered at all**: `next build` prints
  **16 `ƒ` routes + the three `○` icon files** (19 entries), and the last `○` HTML route,
  `/_not-found`, became `ƒ` when `app/not-found.tsx` took `await connection()` — see the trap below,
  which is why. *(The 「19 `ƒ`」 that `P11.F3`'s own notes recorded miscounted the table: 19 is the
  number of **entries**, of which three are the icon handlers. Re-read off the build at
  `P11.REVIEW`.)*
...
- **The global footer publishes *served* config, and the read lives in the root layout** (P11.F2).
  ...
  - ~~**Route kinds did not move**~~ — **superseded by `P11.F3`**: that was true of F2's own change,
    but `/_not-found` became `ƒ` two slices later, so nothing is prerendered any more (see *Stack*).
    `/ops` still renders no footer, though the values do travel in that route's flight data, because
    the root layout hands them to the client chrome on every route.
  - **「unreachable → no contact line」 is about a *cold* cache.** The read is `revalidate: 600`, so
    with a warm ISR entry the footer keeps printing the last-known contact while the API is down —
    measured at `P11.REVIEW` in a production build. That is what a 10-minute cache is for; the
    no-line behaviour is what a reader meets when the entry is cold or the value is unset.
...
- **No prerenderable tree may render a request-dependent client value** (`P11.F3`, found at the third
  P11 gate). `app/not-found.tsx` is a Server Component that Next renders both for a segment's own
  `notFound()` and for any unmatched URL, and its `RequestedPath` child reads `usePathname()` —
  the documented way to echo the address on that page. Prerendered, the only pathname that exists at
  build time is Next's own internal segment name, so the build baked the literal string
  **`/_not-found`** into `_not-found.html` *and* into its flight payload. Every production reader who
  mistyped an address was therefore shown **`/_not-found`** — a Next internal name, on the one screen
  whose entire job is to echo the address *they* supplied (R10 §8) — and then React hydrated against
  their real path, threw **#418 (server rendered text didn't match)** and regenerated the tree.
  - **It was invisible in dev and invisible to a route-by-route sweep.** `next dev` renders the page
    per request and so never disagrees with itself, and a production sweep that does not include a
    404 URL sees nothing. It survived two phase reviews on exactly that gap.
  - **The fix is `await connection()`, not suppression.** `suppressHydrationWarning` on the path
    element would have *kept the server's wrong string on screen*, because React does not patch
    suppressed text. Stating that the page depends on the request is the truth: the server renders the
    reader's own path on the first paint, at the cost of one render per 404.
  - **The standing rule:** if a value can only exist at request time, the tree that renders it must be
    request-time too. Grep for `usePathname` / `useSearchParams` in anything a build could prerender.
  - Re-verified at `P11.REVIEW` in a production build: **8 distinct unknown URLs** each echo their own
    path off the wire on the first paint, and 56 production loads over seven routes at 390 and 1280 —
    with Chrome's pre-hydration attribute injected — report **0** hydration messages. The negative
    control (a text mismatch planted in the same element before hydration) still throws
    **#418 `args[]=text` 4/4**, so that zero is a measurement and not a blind detector.
- **`<html>` carries `suppressHydrationWarning`, and that scope is the whole point** (`P11.F3`).
  Chrome writes `__gchrome_remoteframetoken="…"` onto the root element after the document is parsed
  and before React hydrates, so the client element carries an attribute no server rendered. Nothing
  server-side prevents it, so the flag silences **the browser, not the product**. React accepts a
  mismatch only when the flag sits on the element itself (attributes) or on the host parent of a text
  node — it is **not** inherited — so it hides nothing below `<html>` and must never be copied
  downward. The cost it does carry, accepted deliberately: a genuine `lang` or font-variable mismatch
  on that one element would also be hidden, and all three are literals with no runtime input.
  - **Proved with controls, not reasoned about, and re-proved at `P11.REVIEW`:** with the flag in
    place the same injection aimed at `<body>` still fires (2/2 routes), and a planted deep text
    mismatch still throws #418 in a production build (4/4). A suppression that quietly hides more than
    it claims is the failure mode; these two controls are how you show it does not.
