...
doc_id: frontend
version: v0008
created_at: 2026-08-30T03:12:27+09:00
source: P10.REVIEW
summary: P10 rebrand — the chrome loads the operator mark through WORDMARK_WHITE, the ring vocabulary and the four mijual-*.png retire, and the favicon gap re-opens as a superseding row
previous: v0007_p9_r16_the_ask_re-cut_to_one_column_with_a_start_screen_five_new_answer_elements_drawn_by_one_renderer_a_keyed_block_store_and_three_retirements
---
...
**The wordmark is the operator-delivered mark, `juju-wordmark-white.png`, rendered
...
intrinsic pair `WORDMARK_NATURAL = {1213, 319}` (the `RING_*` vocabulary is gone with the asset).
**The four binary assets** — `juju-logo-source.png`, the black and white wordmark derivations, and
`PretendardVariable.woff2` — are in the repo byte-for-byte. **Provenance is now three classes, not
...
re-deriving. Pretendard is verified *drawing* Korean prose in a real browser, not
merely 200-ing.

**The signed heights `h19` (nav) / `h17` (footer) did not change, and the new mark is a different
shape inside them.** The `<img>` computes to **72.24×19** and **64.64×17** (dev and a production
build agree to the hundredth of a pixel), so the nav slot narrowed from the retired ring's **119.6px**
to **72.24px** — the 24px gap is unchanged and nothing wraps or overflows at 1280 or 390. The ink is
bottom-half only: the Korean glyph band is **162/319** of the box, i.e. **9.65px** at h19 and
**8.63px** at h17, against a 13.5px nav label and a 12px footer source line. **The brand is therefore
0.72× the type beside it, where the ring was 1.07×** — legible (clean at retina 2×, neutral white, no
halo, measured channel spread of 1) but the smallest text in its own bar, and in the footer the typed
`© 주주의관제탑` is the larger of the two settings of the same name. Raising the pair to `h27`/`h24`
would restore parity (13.71px / 12.19px, boxes still narrower than the ring's) — **a deviation from a
signed number, so it is the operator's call at the acceptance gate, not a slice's.**
...
- ~~Binary assets~~ — **re-closed 2026-08-30 (P10)** by the operator's new mark; the four in the repo
  are byte-for-byte, and the two derivations are regenerable from the recorded command. There is
  still **no SVG wordmark and no favicon** — the natural wordmark pair is **1213×319**, the mark does
  **not** reduce legibly to 32px (an open operator decision: ship none, supply a square symbol
  export, or authorize cropping the sparkle cluster), and re-encoding a delivered asset is not an
  implementation call. Nothing was substituted, generated or placeheld: there is no `app/icon.*`, no
  `favicon.ico`, no `manifest`, no `openGraph` and no `icons` key, and **zero** `link[rel*=icon]` in
  dev or in a production build.
