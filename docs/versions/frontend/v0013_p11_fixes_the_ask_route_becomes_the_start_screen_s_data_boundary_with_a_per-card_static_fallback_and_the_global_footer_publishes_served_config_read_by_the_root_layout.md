...
source: P11.REVIEW
summary: P11 fixes: the ask route becomes the start screen's data boundary with a per-card static fallback, and the global footer publishes served config read by the root layout
previous: v0012_p11_the_ask_becomes_a_conditionally_mounted_absolute_popover_and_the_two-part_rule_for_anything_placed_inside_prose
---
...
- **`components/Citation.tsx` is an overlay popover on every surface** — 32px trigger desktop / 44px
  ...
  move.**
- **주주의관제탑 owns its 404.** `app/not-found.tsx` + `RequestedPath.tsx` render the Korean not-found for
...
- **The vocky script seam is gone.** `VockyTrigger`, `VockyScript`, every `data-vocky-trigger` and
  `NEXT_PUBLIC_VOCKY_SRC` are deleted; `components/chrome/Feedback.tsx` is the product's own 의견 surface
...
| R2 ring logo assets (`mijual-logo-ring-{charcoal,white}.png`) | **P10 (operator's mark)** | `juju-wordmark-{black,white}.png` — the Korean wordmark **주주의관제탑** with a sparkle cluster and **no ring**; the favicon-scale gap is **open again** (the mark does not reduce legibly to 32px) |
| R1/R2 English wordmark **alone** | **P10 (operator)** | the unspaced **주주의관제탑**, everywhere — `MIJUAL` and `MIJUAL OPS` are retired with **no romanized replacement** |
| P10 `juju-wordmark-{black,white}.png` (thin first delivery) | **R17 (`P10.S6`, operator-supplied second delivery)** | `juju2-wordmark-white.png` — trimmed **1292×371** (**superseded by R18 → 1247×371**, below), 2.3× the ink of the retired pair; the black variant is **not** re-derived (nothing referenced it, and the symbol is painted by CSS `mask`) |
| R1 "no favicon-scale symbol mark" gap, **re-opened by P10** | **R17** | **closed** — the operator delivered a square symbol export; `app/icon.png` 32 / `icon1.png` 16 / `apple-icon.png` 180 on an opaque `#0a1310` tile, ink width 84 % of the box (**both superseded by R18**, below) |
| R2 §Page shell wordmark **h19 / h17**, box-centred | **R17** | **h27 / h24**, **the heights stand**; the *offset* R17 signed was **superseded at the P10 round-3 gate** (row below) — one placement at every viewport, no breakpoint |
| R6 launcher mark (22×22 Saturn, 4.5 s band + 14 s ring drift) and R6's ambient-motion exception | **R17** | a **32×32 sparkle painted by CSS `mask` + `currentColor`**, **zero animations**, hover response is colour; the exception is **expired** |
| R17 wordmark trim box **1292×371** | **R18 (`P10.review`, applied by `P10.F1`)** | **1247×371** — a 45-column quarter-em space between 「의」 and 「관」 spliced out of the derivation command; **the artwork did not change** and every vertical constant is untouched |
| R17 favicon = opaque `#0a1310` tile + white ink | **R18 (applied by `P10.F1`)** | a **transparent** tile carrying a single `#2b8e6c` ink — the old rule was not wrong, its premise was: the background was carrying contrast for an ink pinned to white |
| R17 ink width **84 %** (favicon and launcher, one number) | **R18 (applied by `P10.F1`)** | favicon **75 %**, **launcher stays 84 %** — same artwork, different surface, deliberately divergent placement rules |
| R2 `.link { display: inline-flex }` (active = 600 + 2px underline, and the sibling moves) | **R18 §②/§②b (applied by `P10.F2`)** | nav `.link` and `/ops` `.tab` are **`inline-grid` + a `visibility: hidden` 600 `::after` twin**, so an active tab reserves its own width and shoves nothing |
| R17 `INK_OFFSET 0.2628 × H` — the band on the **row's** optical centre (`translateY(-7px)` / `-6px`), re-confirmed unchanged by R18 | **the operator, at the P10 round-3 acceptance gate (applied by `P10.F3`)** | **text-referenced**: the band's ink bottom sits on the **neighbouring Hangul's** ink bottom — `INK_OFFSET_PX = {27: 8, 24: 6}` → `translateY(-8px)` / `-6px`. The nav number moves by 1px; the footer's is re-derived and unchanged. **h27 / h24 and everything else R17 signed are untouched** |
| R1 `foundations/fonts.css` + `PretendardVariable.woff2` (class A, "do not edit") | **P10.S7 (operator instruction)** | **deleted** — a self-hosted Noto Sans KR subset via `next/font/local`, with the IBM Plex Mono CDN `@import` gone with it |
...
- **Styling:** `public/foundations/tokens.css`, **vendored byte-verbatim** from the landed record
  (diffed identical, only a provenance header prepended) and **served as a static file, not bundled**.
  It is read-only: a change in it is a design change, and P10 did not touch it (R17 `Token delta:
  None`). Its sibling `fonts.css` — the other class-A R1 export — **was deleted by `P10.S7`** under
  operator instruction, with its sha256 recorded in `public/assets/README.md`; the two font tokens it
  defined are now overridden in `app/shell.css`. **The override selector is `html:root`, not `:root`,
  and that is load-bearing**: `layout.tsx` links `tokens.css` from `<head>` and React places it
  *after* the CSS Next hoists for the bundle, so a bare `:root` ties on specificity and loses on
  source order — silently leaving the product on a face that no longer exists on disk.
...
- **Noto Sans KR** (self-hosted subset) for Korean UI; **IBM Plex Mono for every numeral**
  (금액·주수·%·dates·D-day·rcept_no) at ~0.95em of the surrounding sans. **Korean prose is never mono.**
  Both faces ship through `next/font/local` from `app/fonts/` — Pretendard Variable and the Google
  Fonts CDN are both retired (P10.S7; payload and provenance in `architecture.md`). Verified in a
  real browser at 1512/1440/1280/768/767/481/390: **no mono date or figure splits across lines** and
  no surface overflows horizontally, so the metric change did not move any signed line break.
...
- Motion: **fades only**, 120/200/320ms, one ease `cubic-bezier(.2,0,.2,1)`; countdown colon blinks
  ...
  stars stop or hide.
- ~~**One sanctioned ambient-motion exception:** the AI 질문 launcher mark (22px Saturn, rotating
  band 4.5s, ring split front/back on one 14s drift).~~ **Expired at R17.** The Saturn, both its
  animations and the two-half-ring construction are **deleted**; the launcher now carries a 32×32
  sparkle painted by CSS `mask` + `currentColor` and **zero animations in every state** (rest, hover,
  active, focus-visible, open, reduced-motion — measured with `getAnimations()`, `[]` in all six).
  Hover response moved to **colour** (`#eaf2ed` → `--live`), which is deliberately kept under
  `prefers-reduced-motion` because colour is not motion and it is the only hover response left.
  **This does not make the product motion-free:** R2's cosmos backdrop (`drift` 80s, `twinkle`,
  `shoot`), Hero's 26s `orbit` and the streaming caret all remain by their own signatures. Under
  `prefers-reduced-motion` the whole document reports **zero** animations.
...
**That corner is shared, and sharing it is a footer rule, not a launcher one.** The launcher is
`right/bottom: var(--space-6)` at 68×50, so it occupies **92px** from the viewport's right edge —
which is exactly where the footer's action row ends, and it covered 「의견 보내기」 outright at every
desktop width below the crossover (a dead interaction: `Feedback.tsx` anchors its 380px panel on that
button). R17 fixes it twice over, and both halves are needed: the duplicated desktop 「AI 질문」 footer
link is **hidden at ≥768** under R8 §1's "the same destination is not said twice", and the footer
reserves the corner with `padding-inline-end: 108px` scoped to **768–1255px**. **108 = 92 + 16**, not
84: `.content` sets `padding-inline: 24px` as a *shorthand* and `Footer.tsx` puts `.content` and
`.inner` on **one element**, so the reservation **replaces** that 24px rather than adding to it, and
clearance is `P − 92`. Measured live at 768 / 1024 / 1120 → **16px** of air, 1255 → 83.5, 1256 → 0
(the crossover, flush), 1280 → 12; the button answers `elementFromPoint` at all four corners and its
centre at **every** width, and a real click opens the panel fully inside the viewport. Two
consequences worth not "simplifying": the selector must be **`.inner:global(.content)`**, because
`app/shell.css` wins the (0,1,0) tie on source order and a bare `.inner` leaves the computed padding
at 24px at every width; and **≤767 keeps the 「AI 질문」 link**, since the launcher does not render
there and it is that destination's only footer entry.

**The wordmark is the operator-delivered mark, `juju2-wordmark-white.png`, rendered
height-constrained through a plain `<img>` — never `next/image`**, which would ship a re-compressed
derivative of an asset that is never re-encoded. It is loaded through `WORDMARK_WHITE` at the
intrinsic pair `WORDMARK_NATURAL = {1247, 371}` (the `RING_*` vocabulary is gone with the asset).
**The four binary assets** — two operator deliveries (`juju2-logo-source.png`,
`juju2-symbol-source.png`) and two repo-generated white derivations
(`juju2-wordmark-white.png` 1247×371, `juju2-symbol-white.png` 222×165) — are in the repo
byte-for-byte; the three favicon tiles under `app/` are derived from the second of those. **Provenance is now three classes, not
one, and `frontend/public/assets/README.md` is the authority on which file is which** (P10.S1): a
**design-project export** and an **operator delivery** are both untouchable and not regenerable here,
so replacing one means a new file from outside, never a local edit; a **repo-generated derivative**
is the sanctioned exception — the white variant is an alpha-preserving black→white recolor of the
operator's own file by one recorded ImageMagick command, and it is verified **by pixel signature,
never by file sha256**, because ImageMagick stamps `png:tIME` into every write. Never "fix" a hash by
re-deriving. The shipped face is verified *drawing* Korean prose in a real browser, not merely
200-ing.

**Two guards on the derivations exist because each operator delivery carried a defect invisible
except in the variant the product ships**, and both re-derive on demand from the landed sources: a
filled 「의」 counter survives an alpha-preserving recolor and becomes a solid white blob on cosmos,
and the symbol is **cropped (`-crop 222x165+39+62`), never trimmed**, because two low-alpha ghost
fragments at the delivery's bottom-left have alpha > 0, so `-trim` keeps them and the recolor turns
them into visible smudges. The symbol derivative re-trims to exactly **`222x165+0+0`** (re-measured
`P10.REVIEW` round 3).

**The counter guard is stated here in the form that can fail — the earlier wording could not.**
Rounds 1 and 2 of this doc said "the wordmark's opaque near-white pixel count must be 0 … measured on
the landed files: 0 / 0 / 0 at thresholds 200/240/250". That reading is **0 only on the dark
`juju2-logo-source.png`**. Run unscoped on the shipped *white* derivative — which is what "the landed
files" invites — it returns the opaque-pixel count itself (**69,630**, at every threshold, before and
after R18's splice alike), so it can never be 0 and filters nothing; on `juju2-symbol-white.png` it
returns **1,824**, i.e. it would report failure on a correct file. R18 §① inherited the same broken
row and `P10.F1` caught it there. The working forms, all re-measured at `P10.REVIEW` round 3 against
deliberately corrupted inputs so that each one demonstrably *can* report failure:

- **the two enclosed counter islands are constant** — `50×46+402+226` (「의」's ㅇ) → **481** and
  `69×15+969+335` (「탑」's ㅂ) → **15**, identical before and after the splice (the second island's
  `x` moved −45 with it). Filling the 「의」 counter drives the first to **1442**;
- **the derivative's alpha channel is a pure splice of the source's** —
  `d90e982748259b5356373cb82b5fb9fc20678947eb6df1994554b56ae895df79` for both the shipped file and a
  fresh `+append` of the same two crops of the source trim. The pre-R18 raster hashes differently
  (`296508ff…`), which is what makes the comparison a test;
- **the favicon ink is one colour** — count non-transparent pixels whose RGB is not `(43,142,108)`:
  **0** in all three tiles (84 / 37 / 1,117 visible pixels). A wrong ink colour reports 84/84 and a single stray
  half-white pixel reports 1/85. R18 §③'s signed form
  (`%[fx:int(255*u.r)]`) reads pixel (0,0) — transparent canvas — and answers `0,0,0` on the good
  tiles *and* on both corrupted ones.

`frontend/public/assets/README.md` carries all three commands verbatim and a paragraph forbidding the
broken forms' return.

**The heights are `h27` (nav) / `h24` (footer), and the vertical placement is *text-referenced*** —
**the mark's glyph band sits on the same baseline as the Hangul standing next to it.** The heights are
R17's, unchanged; the *law* is `P10.F3`'s, and it **supersedes R17's `INK_OFFSET 0.2628 × H` by
operator instruction at the P10 round-3 acceptance gate** (R18 had explicitly re-confirmed the old
value; see `decisions.md` D-30). R17 derived its offset from the **image alone** — the glyph band's
geometric centre sits at 76.28 % of box height — and so aligned the band to the optical centre of
**the row**, never to the type beside it; the operator saw the result and reported the mark reading
low («로고 글자가 옆 nav 링크 글자보다 아래로 내려가 있다»).

The `<img>` computes to **90.75×27** and **80.66×24** (R18's narrower box; the arithmetic is
`1247/371 = 3.3612`, and Chrome quantises the footer width to 80.66), dev and a production build
agreeing to the hundredth of a pixel, at **every** viewport — there is no breakpoint on the mark.
**The offset is a module constant inside `Wordmark.tsx` (`INK_OFFSET_PX = {27: 8, 24: 6}` →
`translateY(-8px)` / `-6px`), not a call-site prop**, so no consumer can forget it. It exists because
the mark's ink is bottom-half only: the Korean glyph band is the bottom **47.44 %** of the box (ink
rows **195–370** of 371, flush to the bottom) with the sparkle cluster flush to the top-right and
**30 empty rows** (165–194) between, so centring the *box* sits the *name* low.

**"Baseline" here means the Hangul syllable-block bottom, never the alphabetic baseline.** Rendered
Noto Sans KR carries real ink *below* its alphabetic baseline — measured on the element's own computed
font, **1.05px at 400 / 1.16px at 600 at 13.5px, and 1.02px at 12px** — so aligning the band to the
alphabetic baseline paints the mark ≈1px **high**. The other law the operator named, a shared optical
*centre* line, is **rejected**: the band (12.81px at h27) is taller than the label ink (11.73–12.32px),
so that law lands anywhere between **7.38 and 7.69** depending on which label you point it at, and
against the pure-Hangul labels it rounds to **7** — i.e. it re-derives the very placement that was
rejected. A law that cannot decide, and whose answer to the complaint is "no change", is the wrong law.

Measured in the running product (dev **and** a production build, 1280 and a true 390, by two
independent methods — canvas `TextMetrics` on the element's own computed font, and an 8× pixel scan of
band-only columns — agreeing to 0.04px):

| surface | neighbour | its Hangul ink bottom | band ink bottom | Δ |
|---|---|---|---|---|
| nav h27 | `.link` `AI 질문` 400 / 600 | 30.875 / 31.125 | **31.000** | +0.125 / −0.125 |
| nav h27 | `.link` `보유 종목` 400 / 600 | 31.125 / 31.125 | **31.000** | −0.125 |
| footer h24 | `.source` 12px | row-relative | same row | **0.00 – 0.28** |
| 390 nav | the **메뉴** button (reported, not judged) | 30.833 – 30.875 | **30.958** | +0.083 / +0.125 |

The nav bar is `height: 52px` **border-box** with a 1px bottom rule, so the content box `.inner`
stretches into is **51px**; at `-8` the mark's box top lands at **4.00px** (top clearance 4.00, was
5.00) and its bottom at **31.00**. The sparkle occupies the box's top ~11.9px and comes nowhere near
the bar's hairline at y 51–52. **The footer's number does not move**: at h24 the band was already
within 0.28px of `.source`'s Hangul bottom, so the same law re-derives the shipped `-6`.

**Two traps for anyone re-measuring this.** (1) **Chrome pixel-snaps the mark's paint origin** to
whole CSS pixels when its layout y is fractional (the footer lands at e.g. 745.83 / 746.48 depending on
route and is painted at 746), so the footer's *painted* residual varies by up to ±0.5px with page
position and no single offset can remove it — a `getBoundingClientRect()` reading and a screenshot will
disagree by that much, legitimately. (2) A zero-size `inline-block` baseline probe is **invalid inside
a flex or grid container**: it becomes a centred item and returns the box centre, not the baseline.
`.link` is an `inline-grid` with `place-items: center`, so its `<span>` is blockified — wrap the text
first and assert the host's rect is unperturbed.

**The check that guards this, and the input that makes it fail.** *On every surface where the mark
shares a row with Hangul type, the band's ink bottom is within **0.5px** of that type's Hangul ink
bottom* (0.5px being half the integer-px rounding quantum, so the tolerance is exactly what the
rounding rule can leave behind and no more). Re-painting the mark at R17's `-7` makes it report
**FAIL on every nav and footer surface, worst 1.125px**, in dev and in a production build alike. At
**390 the check is N/A by construction** — the nav's labels are `display: none` and the footer's
`.identity` stacks to a column, so no Hangul label shares a row with the mark; the 메뉴 button is
**reported, never judged**, because the two measurement methods disagree there by ~0.5px.

**A known, deliberate consequence: the bar's own type is not on one line with itself.** `.utility`'s
「로그인」 is `align-items: center` in the full 51px box while `.link` stretches and reserves a 2px
transparent bottom border, so its cell is centred in **49px** — the account slot's Hangul ink bottom
measures **31.75** against the links' **30.875–31.125**. This is **pre-existing since R2/R8** and was
not created by `P10.F3`; but because the mark is now aligned to the **links** (the operator's own
stated reference), the mark sits **0.75px above** 로그인 where under R17's law it sat 0.25px below.
Bringing `.utility` onto the links' line is a design decision and is **open** — it is not a defect to
be silently "fixed".
...
- **`/ask` is one `max-width: 760px` column.** The 340 rail and its two-column grid are **deleted**.
  ...
  sentence **verbatim** when pressed. (The set went to six at `P11.S2` and back to four at
  `P11.F1` on the operator's gate report; two of the four now carry a **server-picked** company —
  see the data-boundary entry below.) A thread turns the page into 스레드 + a bottom-sticky composer
...
- **The chip's anatomy is `Citation.tsx`'s R10 one: a conditionally mounted, absolutely positioned
  popover** (P11). Until P11 `InlineCitation` mounted its quote panel after **every** chip and
  `Ask.module.css` gave that panel `display: grid` (`grid-template-rows: 0fr` + `overflow: hidden`,
  held shut with `inert` and animated open by height). A block-level box inside `<p class=prose>`'s
  inline formatting context splits the paragraph into anonymous block boxes, so **every chip forced a
  line break** — a sentence resting on three 근거 rendered `…입니다.[1] ⏎ [2] ⏎ [3]`, and
  `.sentence + .sentence`'s `.25em` gap, which assumes inline siblings, was inert with it. The
  popover now mounts **only while open**: the prose wrap is `position: relative; display: inline-block`
  (`.citationWrap`), the row wrap keeps `display: contents` and the popover anchors to `.row`, which
  took `position: relative` — so R16 §2.6's 「행 아래, 블록 전폭」 survives (measured 732px at 1280,
  354px at 390) and the value-column collapse above is **structurally impossible**, because an
  absolutely positioned box sizes no grid track. `.quoteWrap` / `.quoteClip` / `.quotePanel` /
  `.quoteOpen` and `inert` are retired: an unmounted popover's DART link is unreachable by
  construction (measured: `a[href*="dart"]` 3 → 4 on open, back to 3 on close).
  - **The popover's ground is opaque — the one deliberate deviation from R16.** The round signed
    `--surface-inset` for an *in-flow* panel; as an overlay that translucent token lets the sentence
    read through the reader's evidence. The 2px `--live` left edge and the 180px quote cap stand; the
    fill is the product's own overlay ground (`#0e1a15` + 1px `--border-strong` + `--panel-glow`, the
    widget's and `Citation.module.css`'s). No new colour. Measured open in dev and production:
    `rgb(14, 26, 21)`, `border-left: 2px rgb(95, 208, 165)`, `z-index: 40`, quote `max-height: 180px`.
  - **Close model:** 칩 재탭 (R6-4) + press outside + Esc with focus returned to the chip (R10 §6).
    The primitive's `×` is **not** adopted — it needs a head row and a 28/44px control that R6-4 never
    drew on a 10px chip.
  - **`fit()` measures the nearest clipping box ∩ viewport, not the window, and flips upward.** The
    620px widget thread is an `overflow-y: auto` box, so a chip in the last answer opened its evidence
    below the fold; the popover flips above the chip (`data-flip="up"`) when there is more room there,
    and clamps horizontally to the same box so a prose popover no longer hangs over the widget's right
    border. On `/ask` there is no clipping ancestor, so it is the primitive's plain viewport clamp.
- **`app/ask/page.tsx` is the start screen's data boundary, and `/ask` is a dynamic route** (P11.F1,
  superseding that file's own 「no data loading」 docstring). The two 공시 start cards must name a
  company the corpus can still answer about, so the route reads `GET /ask/start-cards` on the server
  and hands `AskPage` the resolved sentences as a prop. It is deliberately **request-time**:
  `connection()` + `cache: "no-store"`, which moves `/ask` from `○ (Static)` to **`ƒ (Dynamic)`** in
  `next build`'s route table — that route kind *is* the check, because a statically rendered start
  screen is the stale-card defect wearing a different mask (verified in a production build: three
  consecutive requests to one running server, upstream toggled between them, rendered three different
  card sets).
  - **The fallback contract is durable, and it is per card.** A 2.5s timeout, any error, and a `null`
    slot fall back to `copy.ts`'s static `START_CHIPS_KO` **one card at a time** — so a single empty
    slot leaves the other card's served company alone. The empty state therefore renders **four cards,
    always**: no spinner, no blank grid, no hole where a card goes (measured with the API stopped:
    200 in 25ms, four cards, two even rows). The static array is documented as *the screen must not
    be empty* set, not as the specification — it names companies and it ages, and that is accepted
    because it only ever renders when the same service is down anyway.
  - **No Korean crosses the wire.** The endpoint answers companies (`corp_name` / `corp_code` /
    `filings` / `rcept_no` / `dday`), never sentences; `components/ask/copy.ts` keeps its standing
    role as the single home of every Korean string, holding the two card templates with a company
    slot. The new client seam lives in `lib/api.ts` + `lib/types.ts` like every other route path.
- **The global footer publishes *served* config, and the read lives in the root layout** (P11.F2).
  `SiteChrome` is a client component (it branches on the pathname for `/ops`), so nothing under it can
  read the API on the server — `app/layout.tsx` is the nearest server component and the one place the
  read happens once for every route. It calls `getSiteContact()` with `next: { revalidate: 600 }`, a
  2s timeout and `.catch(() => null)`, and passes a plain serializable prop down. **Cached, unlike
  F1's cards**, and for the opposite reason: the contact changes almost never and the footer renders
  on every page, while a stale card is the defect the cards exist to fix.
  - **Absent or unreachable renders no contact line at all** — never an empty label, never a
    placeholder, and never the agent's 「미정」 voice, which belongs to `get_contact` and not to the
    chrome. Verified in a production build against an upstream answering nulls: the row reads
    「자료: 금융감독원 DART 전자공시」 and the page contains no `mailto:`/`tel:` at all.
  - **The typography is an operator override of R8 §4, recorded as one in `Footer.tsx`.** R8 deleted
    four sentences at the operator's earlier 「simple and clean」 and typeset the surviving row in the
    UI face *because* nothing in it was numerals — and a phone number is numerals. So the email is
    sans (12px) and the phone is **mono** (11px, Plex Mono running optically larger at the same
    size), both joined to the **existing** 자료/© row (no second row returns), with no invented Korean
    label; they are `mailto:`/`tel:` links because the values are for using, and the `tel:` href keeps
    the operator's own hyphens. R8's five unrendered constants and P8 Q5 are untouched.
  - **The footer is taller between ~481 and ~840px than it was.** The identity line grew by ~215px,
    so `.inner` wraps its action row earlier: measured **73px** at ≥~840, **118px** at 768–820,
    **132px** at 481–767, unchanged at ≤480. Nothing overlaps and nothing overflows at any width, and
    「의견 보내기」 still answers `elementFromPoint` at every measured width — but it is a visible
    change to a signed surface and was shown to the operator rather than buried.
  - **Route kinds did not move**, and `/ops` still renders no footer — though the values do travel in
    that route's flight data, because the root layout hands them to the client chrome on every route.
- **Everything inside `<p class=prose>` must be phrasing content *and* inline-level — it is a two-part
  rule, and each half has now failed on its own.** R14 walk finding 3 was leading whitespace in the
  streamed text, which made every continuation sentence look indented (fixed at the store boundary in
  `lib/ask.ts` `leading()`); P11's was a block-level box in the inline flow, which broke the line after
  every chip. **The symptom was identical both times — "a line per sentence" — and the cause was not.**
  Anything new placed in an answer's paragraph is checked against both halves before it is believed.
...
- **An active tab must reserve its own bold width, or it shoves its siblings** (R18 §②/§②b,
  `P10.F2`). `.active { font-weight: 600 }` changes the *label's* width, so switching routes moved
  the next link's start point: the nav's `보유 종목` slid **0.703125px** (`279.484375` on `/ask`
  against `278.78125` everywhere else) and `/ops`'s three right-hand tabs slid **0.3125px** on
  `/ops/accuracy`. Only labels with a **latin glyph or a space** ever moved — Noto Sans KR's Hangul
  advance widths are identical at 400 and 600, which is why `보유 종목` measured 53.469 at both and
  why a pure-Hangul strip shoves nothing at all. The fix on both surfaces: `display: inline-grid` with
  one named area, the label in a `<span>`, and a `::after` twin carrying `attr(data-label)` at weight
  600 in the *same* cell, so the cell is always the 600 width. Both arrays are now identical to the
  decimal across all five nav routes and all six `/ops` routes, dev and production.
  - **`visibility: hidden`, never `opacity: 0`** — measured with a negative control rather than
    reasoned about, twice independently (`P10.F2`, then `P10.REVIEW` round 3): under `opacity: 0` the
    accessibility tree names the link **`AI 질문 AI 질문`** and all six `/ops` tabs double likewise;
    `height: 0` and `overflow: hidden` do **not** keep generated content out of the AX tree. A later
    "simplification" to `opacity: 0` breaks the a11y contract while leaving the layout correct.
  - **`/ops` takes the rule *without* `white-space: nowrap`**, deliberately. At 390 those six tabs
    rely on wrapping to fit the bar; injecting `nowrap` on the shipped build widens the strip
    `154.531 → 380.828` and pushes the last tab to `right: 439.875`, and it buys no reservation there
    anyway (the cell never reaches max-content), so it would trade a filed defect (**D24**) for a
    worse one. The nav keeps `nowrap` as R18 wrote it.
  - **Two things a later checker must not assert.** The computed `display` reads **`grid`**, not
    `inline-grid`, on both surfaces — flex-item blockification, the same reason the old `.link` read
    `flex` rather than `inline-flex`. And `place-items: center` centres the shorter (400) label in the
    reserved cell, so an inactive label's glyphs sit ~0.35px right of where they used to; that is
    inherent to the prescription and is what makes the *box* stable.
  - **Two surfaces are outside the rule, on purpose.** The ≤480 sheet's rows are full-width, so weight
    shoves nothing (`::after` content computes to `none` there — verified, not assumed). The landing
    board's tab strip (`Board.module.css`) still carries the identical defect and was **not** fixed:
    its tabs render three children, which one `data-label` twin cannot reproduce, and `.tabs` is
    `overflow-x: auto` on mobile. Measured rather than guessed: **zero shift at 1280** (the desktop
    labels are pure Hangul) and **one 0.42px nudge at 390**, on `CB`, the only latin label. It is an
    open operator decision, not an oversight.
...
- ~~Binary assets~~ — **re-closed 2026-08-31 (P10 round 2)**. The operator's second delivery
  supersedes the first, and the **favicon gap that had stayed open since R1 is closed**: they
  supplied a square symbol export, so nothing was cropped out of the wordmark and the README's
  standing rule (*no image is substituted, generated or placeheld*) is **satisfied, not relaxed**.
  `app/icon.png` 32 / `icon1.png` 16 / `apple-icon.png` 180 ship through Next's `app/` file
  conventions — no hand-written `<link>` exists. **R18 re-cut the tiles (round 3, `P10.F1`):** they
  are now **transparent**, carrying a single ink literal **`#2b8e6c`** at **75 %** ink width on
  integer-centred margins — measured ink trims `24x18+4+7` on the 32 and `134x100+23+40` on the 180,
  `opaque=false` on all three, and **every visible pixel exactly `#2b8e6c`**. R17's opaque `#0a1310`
  tile was not a wrong rule, it was a rule whose premise was the *white* ink: unpin the colour and the
  background stops being needed. `#2b8e6c` reads on both sides — WCAG contrast **4.05** on a white
  tab, **3.98** on Chrome's dark one, recomputed independently at `P10.F1` and again at
  `P10.REVIEW` round 3. **The launcher deliberately keeps 84 %** (`Launcher.module.css`,
  `mask-size: 84% auto`, verified in the running product): same artwork, different surface, and that
  divergence is the record, not drift. **The 16 is a 2:1 `-filter Box` downscale of the 32 raster, not
  separate artwork** — the default Lanczos rings into a colour the design never specified. Its
  softness (five dots at ~1.2px at 75 %, ~1.4px at 84 %) is a **recorded limitation**, and R17's
  refusal of the single-star crop stands. Verified served, `link[rel*=icon]` × 3, on every reader and
  `/ops` route in dev **and** a production build, with the served bytes hashed against the files on
  disk; re-rendered at the tab strip's own 16 CSS px on white / `#f1f3f4` / `#202124` / `#0a1310`, the
  ink is present on all four (34–37 of 256 pixels differ from the background). **Nobody has yet seen a
  tile inside a real OS tab strip** — neither `P10.F1` nor `P10.REVIEW` round 3 has Screen Recording
  permission, and CDP captures page content only — so that one claim rests on served bytes plus a
  16px paint, and it is the first item in the acceptance walkthrough. There is still **no SVG
  wordmark**, and `/favicon.ico` itself 404s (the `<link>` tags are what browsers use).
...
- **Copy the record does not contain** — each needs a signed decision, and none was invented: the
  ...
  `이메일`, `비밀번호`, `계정 이전`, `© 주주의관제탑`); and the sample's signed **4건** subline above **five**
  live D-day rows.
