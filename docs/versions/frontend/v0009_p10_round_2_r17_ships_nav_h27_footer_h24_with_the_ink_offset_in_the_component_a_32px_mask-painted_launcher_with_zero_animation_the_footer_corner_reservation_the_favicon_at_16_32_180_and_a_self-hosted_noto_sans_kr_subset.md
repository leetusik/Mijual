...
doc_id: frontend
version: v0009
created_at: 2026-08-31T04:28:49+09:00
source: P10.REVIEW
summary: P10 round 2 — R17 ships: nav h27 / footer h24 with the ink offset in the component, a 32px mask-painted launcher with zero animation, the footer corner reservation, the favicon at 16/32/180, and a self-hosted Noto Sans KR subset
previous: v0008_p10_rebrand_the_chrome_loads_the_operator_mark_through_wordmark_white_the_ring_vocabulary_and_the_four_mijual-_.png_retire_and_the_favicon_gap_re-opens_as_a_superseding_row
---
...
| P10 `juju-wordmark-{black,white}.png` (thin first delivery) | **R17 (`P10.S6`, operator-supplied second delivery)** | `juju2-wordmark-white.png` — trimmed **1292×371**, 2.3× the ink of the retired pair; the black variant is **not** re-derived (nothing referenced it, and the symbol is painted by CSS `mask`) |
| R1 "no favicon-scale symbol mark" gap, **re-opened by P10** | **R17** | **closed** — the operator delivered a square symbol export; `app/icon.png` 32 / `icon1.png` 16 / `apple-icon.png` 180 on an opaque `#0a1310` tile, ink width 84 % of the box |
...
intrinsic pair `WORDMARK_NATURAL = {1292, 371}` (the `RING_*` vocabulary is gone with the asset).
...
(`juju2-wordmark-white.png` 1292×371, `juju2-symbol-white.png` 222×165) — are in the repo
...
except in the variant the product ships**, and both re-derive on demand from the landed sources
(`P10.REVIEW`, independently): the wordmark's **opaque near-white pixel count must be 0** — a filled
「의」 counter survives an alpha-preserving recolor and becomes a solid white blob on cosmos — and the
symbol is **cropped (`-crop 222x165+39+62`), never trimmed**, because two low-alpha ghost fragments
at the delivery's bottom-left have alpha > 0, so `-trim` keeps them and the recolor turns them into
visible smudges. Measured on the landed files: **0 / 0 / 0** near-white at thresholds 200/240/250,
and the symbol derivative re-trims to exactly **`222x165+0+0`**.

**The heights are `h27` (nav) / `h24` (footer), and the placement is ink-aligned, not
box-centred** (R17, superseding R2's h19/h17). The `<img>` computes to **94.02×27** and **83.58×24**,
dev and a production build agreeing to the hundredth of a pixel, at **every** viewport — there is no
...
- ~~Binary assets~~ — **re-closed 2026-08-31 (P10 round 2)**. The operator's second delivery
  ...
  conventions — no hand-written `<link>` exists — on an **opaque `#0a1310` tile** (transparent would
  vanish on a light tab), with ink width **84 %** of the box and the box centred on both axes:
  measured 27×20 at 84.4 % on the 32 and 151×112 at 83.9 % on the 180, none of the three carrying an
  alpha channel at all. **The 16 is a 2:1 `-filter Box` downscale of the 32 raster, not separate
  artwork** — the default Lanczos rings into pixels darker than the tile, a colour the design never
  specified. Its softness (five dots at ~1.4px) is a **recorded limitation**, and R17 explicitly did
  **not** adopt the single-star crop that would fix it. Verified served, `link[rel*=icon]` × 3, on
  every reader and `/ops` route in dev **and** a production build. There is still **no SVG wordmark**,
  and `/favicon.ico` itself 404s (the `<link>` tags are what browsers use).
