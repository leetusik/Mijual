...
source: P8.REVIEW
summary: P8 design polish pass — the vocky key boundary now covers a capture path, a reader's free text leaves the system by design with no contact field, and the PII inset is deleted from the auth screen while the structural guarantee stands
previous: v0004_p6_apply_phase_the_anonymity_promise_implemented_and_asserted_minted-not-derived_session_handles_an_identity-free_rate_limiter_the_honest-unset_operator_contact_and_sessionstorage-only_client_persistence
---
...
- The agent's tool surface is the abuse boundary worth watching: `search_events` / `get_event` /
  ...
  `save_feedback`, and `get_portfolio` resolves only the caller's own session/account — **it takes no
  arguments at all**, so a client cannot name someone else's. There is still **no anonymous write
