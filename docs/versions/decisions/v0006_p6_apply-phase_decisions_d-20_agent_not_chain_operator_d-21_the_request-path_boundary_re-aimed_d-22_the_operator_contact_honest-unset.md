...
doc_id: decisions
version: v0006
created_at: 2026-08-22T23:02:28+09:00
source: P6.REVIEW
summary: P6 apply-phase decisions: D-20 agent not chain (operator), D-21 the request-path boundary re-aimed, D-22 the operator contact honest-unset
previous: v0005_p5_apply-phase_decisions_d-16_session-as-a-row_auth_mechanics_d-17_factors-not-products_d-18_the_free_label_tier_d-19_vocky_read-only_observation_d-15_closed
---
...
than allowed to quietly become false (**D-21**). Later phases follow what is written here, not the
...
- **P6 adds the `agent_turn` task, and it runs `LOW`** — the unlisted-task rule, kept deliberately
  rather than by default, for three reasons: the AI 질문 surface is free and unlimited (R6-5), so
  per-turn cost is the *product's* cost and not the reader's; streaming makes the first token
  reader-visible latency and thinking precedes it; and 인용 강제 / never-compute / 거절 가족 are
  enforced **structurally**, so a cheaper level cannot produce an unverified claim — only a blocked
  one. **Measured:** five live turns at `LOW` blocked **nothing**, and the agent's own ledger records
  the level per call like the extractor's. Raise it if a measurement ever shows tool choice
  suffering; the level is recorded either way. **Measured agent cost: ▷ $0.0093 per turn** (18 calls /
  87,908 tokens over 8 turns, `gemini-3.7-flash` rate card, never billed).
...
- **Decision:** Mijual reads vocky's **Project Feedback API** server-side with a `vk_` key the
...
- **What the loop keeps for itself**, because it must not be left to a model: the visible tool fact
  rows, the citation gate, the five signed refusal families, the 갈 곳 links and footer composed from
  tool results **as data** (so the model never writes a URL — the agent carries no route string at
  all), a structural round/call budget, and the ▷ ledger.
...
- **The agent quotes contract numerals exactly as given** (`3200원` where the page renders
  `3,200원`), because R6 says every number is the verified value as-is and formatting inside prose
  would be the agent transforming a number.
