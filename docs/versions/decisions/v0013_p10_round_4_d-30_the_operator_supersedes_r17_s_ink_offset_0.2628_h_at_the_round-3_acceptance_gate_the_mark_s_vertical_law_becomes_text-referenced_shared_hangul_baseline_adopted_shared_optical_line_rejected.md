...
doc_id: decisions
version: v0013
created_at: 2026-08-31T14:25:37+09:00
source: P10.REVIEW
summary: P10 round 4 — D-30: the operator supersedes R17's INK_OFFSET 0.2628*H at the round-3 acceptance gate; the mark's vertical law becomes text-referenced (shared Hangul baseline adopted, shared optical line rejected)
previous: v0012_p10_round_3_d-29_r18_supersedes_three_r17_values_and_three_of_its_own_verification_procedures_could_not_fail
---
...
### D-28 — The rebrand's second round: a new mark, a first-class symbol, and the type it is set in (R17 + operator direction)

**Date:** 2026-08-31 · **Source:** the operator at P10's first acceptance gate, then design round R17
(`P10.S6`), applied by `P10.S7`. **Status:** signed, shipped, awaiting the re-opened gate.

The first round's mark shipped **too thin**, so the operator did not clear the gate. Four directed
fixes plus one design round followed, and the shape of the answer is worth recording because three of
its four parts were *not* an agent's call:

1. **The mark is replaced, not resized.** `juju2` supersedes the first delivery: the trimmed box is
   **1292×371** against 1213×319 (**R18 later spliced it to 1247×371** — D-29), and the Korean glyph
   band carries **2.3× the ink** (37.5 % coverage
   against 16.1 %). Density, not scale, was the answer to "so thin".
2. **The chrome heights are the design's, not the operator's or a slice's.** Gate question Q1 was
   answered *"just open a design slice, I'll handle there"*, and R17 decided **nav h27 / footer h24
   with ink alignment** (`translateY(-7px)` / `-6px`), retiring R2's h19/h17 and its box-centred
   placement.
3. **The symbol becomes a first-class mark**, painted by CSS `mask` + `currentColor` rather than
   shipped as a coloured `<img>` — one asset, every colour — which is also what closes the **favicon
   gap open since R1**: gate answer (b), an operator-supplied square export, so nothing was cropped
   out of the wordmark and the assets README's *"no image is substituted, generated or placeheld"*
   rule is **satisfied rather than relaxed**.
4. **The typeface is a directed fix, not a design question.** The operator named an existing in-house
   implementation (`changple_web`) to copy, so Pretendard Variable and the Google Fonts CDN retire in
   favour of a self-hosted Noto Sans KR subset — see `architecture` for the payload and the one
   deliberate adaptation (KS X 1001 coverage, because this product renders dynamic company names).

**Two arithmetic errors in the landed R17 record were corrected at apply time, and the record was
not edited.** This is the sanctioned handling — a round's landed record is immutable history — and
both corrections are measured, in `works/phases/active/P10/slices/P10.S7/result.md` §2 and
independently re-measured at `P10.REVIEW`:

- **the footer's corner reservation is 108px, not the record's 84px.** The record states *both* 92
  (prose) and 84 (code) for the same declaration; 84 treats the launcher's 68px width as the floor,
  but the launcher is offset 24px from the edge, so it occupies 92px and clearance is `P − 92`. At
  84px **exactly 8px of 「의견 보내기」 stayed under the launcher** and every hit probe returned the
  launcher — the dead interaction R17 existed to fix, still dead, and it would have shipped looking
  fixed. 108 = 68 + 24 + 16.
- **its absolute nav numbers are 0.5px high**, because they compute from a 52px *content* box while
  the bar is 52px **border-box** with a 1px rule (content 51px). The **relationship** the record
  signs — the glyph band on the bar's optical centre — holds to **0.1px**. No fix; recorded so a
  later reader does not mistake 0.5px for drift.

**Two cascade traps are load-bearing and must not be "simplified".** `html:root` (not `:root`) for
the font tokens, because React places the linked `tokens.css` *after* the bundle's CSS and a bare
`:root` loses on source order; and `.inner:global(.content)` (not `.inner`) for the corner
reservation, because `app/shell.css` sets `padding-inline` as a shorthand and wins the tie. Each is
correct-looking CSS that type-checks, builds, and does nothing. Neither is visible in source review;
both took a browser.

**The mockup gate was waived by the operator** (*"no mockup, just apply directly"*), so the phase's
own acceptance gate is R17's only running review — and per the design doctrine, seeing the product
there and changing their mind is a `changes_requested` plus a new superseding round, not a fidelity
failure.

### D-29 — R18 supersedes three R17 values, and **three of its own verification procedures could not fail**

**Date:** 2026-08-31 · **Source:** design round R18 (`P10.review`), run by **the operator themselves**
during P10's open acceptance gate; applied by `P10.F1` (§①③④) and `P10.F2` (§②②b); re-measured by
`P10.REVIEW` round 3. **Status:** signed, shipped, awaiting the re-opened gate.

The round has no orchestrator handoff — there is no outgoing document, only a returned one
(`docs/reference/design/rounds/18-p10-review/`, read-only) — and the mockup was waived again, so the
acceptance gate is its only running review.

1. **The wordmark box `1292×371` → `1247×371`.** The artwork carried a quarter-em space between 「의」
   and 「관」: the Hangul advance in this mark is 183.0px (measured 주→주), but 의→관 was 228.5px, an
   excess of 45.5px = 0.249em, and it rides on 관·제·탑 identically, so it is one shifted space and
   not a kerning pair. The derivation command now splices out `x=530..574`, strictly inside a
   **70-column, full-height, zero-alpha band** (`x=519..588`, independently re-derived at
   `P10.REVIEW`), leaving 11 and 14 columns of clearance. **The artwork did not change** — the
   three ink statistics are identical, and the derivative's alpha is a byte-exact `+append` of the
   source's. **Nothing vertical moved** *in R18*, which was the point: `INK_OFFSET 0.2628·H`,
   `translateY(-7px)` / `-6px` and the measured h27 band centre **25.60** were unchanged by the splice.
   *(R18's re-confirmation of that offset was itself **superseded by the operator at the round-3
   acceptance gate** — see **D-30**. The nav offset now ships as `-8px`.)*
2. **The favicon: opaque `#0a1310` + white ink → a transparent tile + one `#2b8e6c` ink at 75 %.**
   R17's rule was not wrong; its *premise* was. "A transparent favicon vanishes on a light tab" is
   true of **white** ink, and the background was carrying the contrast for an ink pinned to white.
   Unpin the colour and the tile is unnecessary: `#2b8e6c` measures **4.05** on a white tab and
   **3.98** on Chrome's dark one — it leans neither way. **The launcher deliberately keeps 84 %**
   while the favicon goes to 75 %: same artwork, different surface, divergent placement rules, and
   *that divergence is the record*. "Making them consistent" is the regression.
3. **An active tab reserves its own bold width.** `font-weight: 600` on an active label moved the
   next one (nav **0.703125px**, `/ops` **0.3125px**), and only on labels containing a latin glyph or
   a space, because Hangul advance widths are identical at both weights. The fix — label and a
   `visibility: hidden` `::after` twin in one `inline-grid` cell — is now the rule on **two**
   surfaces. `visibility: hidden` is load-bearing, not stylistic: under `opacity: 0` the AX tree
   reads every label **twice**. `/ops` takes the rule **without** `white-space: nowrap`, and the
   landing board is left alone with its defect measured (0px at 1280, 0.42px at 390) rather than
   guessed at.

**And the part worth recording beyond this phase: the round's *prescriptions* were all correct, and
three of its *verification procedures* were incapable of reporting failure.** `P10.F1` caught all
three and replaced them; `P10.REVIEW` round 3 re-ran each replacement against a deliberately
corrupted input to prove it can fail.

- "opaque near-white pixels = **0**" on the white derivative is just the opaque-pixel count
  (**69,630**) — it can never be 0. It came from a README sentence that was **scoped to the source's
  counter region**, and the scope was lost in transcription. *(The same unscoped wording had also
  reached `docs/current/frontend.md` through this phase's round-1 consolidation; round 3 rewrote it.)*
- the aspect ratio was signed **3.3603** and the render widths **90.7 / 80.6**; `1247/371` is
  **3.3612**, so they are **90.75 / 80.67**, and Chrome paints 90.75 × 27 and 80.66 × 24 in both
  runtimes. One slip in a divide propagated into three signed figures **that nobody divided out**.
- the tile ink-colour check reads `u.r` with no coordinate — pixel **(0,0)**, which on a transparent
  tile is empty canvas. It prints `0,0,0` on the good tiles *and* on a wrong-coloured one.

**The standing lesson, and it is now the phase's defining pattern (five wrong signed values across
R17 and R18, three of them checks):** a verification step is only a verification step if some input
would make it report failure. Ask that of every guard before writing it into a record, and where a
guard is scoped, carry the scope with it — an unscoped copy of a scoped check is the failure mode
that produced two of these three.

### D-30 — The mark's vertical law is **text-referenced**: the operator superseded R17's `INK_OFFSET 0.2628·H` at the round-3 acceptance gate

**Date:** 2026-08-31 · **Source:** the operator, in their own failure report at P10's **third**
acceptance gate; applied by `P10.F3`; verified independently by `P10.REVIEW` round 4.
**Status:** shipped, awaiting the re-opened gate.

**What was superseded, and by whom.** R17 signed `INK_OFFSET 0.2628 × H` and R18 **explicitly
re-confirmed it as unchanged**. It is nevertheless wrong for what it was asked to do, and the person
who found that out is the one whose judgment governs here — the operator, looking at the running
product: «로고 글자가 옆 nav 링크 글자보다 아래로 내려가 있다. R17의 잉크 정렬은 마크를 '바'의 광학
중심에 맞췄을 뿐, '옆 글자'와 맞춘 적이 없다. 텍스트 기준 수평 정렬(공유 베이스라인/광학선)로 바꿀
것.» This is a **supersession by the design authority**, not the correction of a mistake in
execution: R17's derivation is arithmetically right and answers a different question. It read the
**image alone** — the glyph band's geometric centre sits at 76.28 % of box height — and so put the
band on the optical centre of **the row**. The row's centre is not where the neighbouring type's ink
is.

**The new law, in one sentence.** *The mark's Korean glyph band sits on the same baseline as the
Hangul standing next to it* — band ink bottom on the neighbour's Hangul ink bottom, computed per
surface, rounded to whole pixels (R17's own rounding rule, which stands). It ships as
`INK_OFFSET_PX = {27: 8, 24: 6}`: the nav moves `-7 → -8`, and **the footer's `-6` is re-derived from
the new law and comes out unchanged**. One law, two numbers, no viewport branch.

**"Baseline" means the Hangul syllable-block bottom, not the alphabetic baseline.** The plan that
commissioned the fix assumed Hangul carries essentially no descender; measured on the rendered type it
carries **1.05px at 400 / 1.16px at 600 at 13.5px, and 1.02px at 12px** (canvas `TextMetrics` on each
element's own computed font, reproduced independently at the review). Taking the *alphabetic* baseline
literally therefore paints the mark ≈1px **high** — visibly. For Hangul beside Hangul the shared line
the eye reads is the block bottom.

**The shared optical *centre* line — the operator's other named candidate — is rejected, and the
reason matters.** The band (12.81px at h27) is taller than the label ink (11.73–12.32px), so that law
lands anywhere from **7.38 to 7.69** depending on which label it is pointed at, and against the
*pure-Hangul* labels it rounds to **7** — i.e. it re-derives exactly the placement the operator
rejected. A law that cannot decide, and whose answer to the complaint is "no change", is the wrong law
for this question. In the footer the two laws agree on 6, so there the choice was moot.

**What did not change.** `h27` / `h24`, the 1247×371 binary, the render widths 90.75 / 80.66, the
horizontal geometry, the favicon tiles at 75 %, the launcher at 84 %, the width reservation on nav and
`/ops`, the two cascade traps. **Top clearance narrows from 5.00px to 4.00px** and was checked against
the blocking threshold (< 2px, or visible crowding of the bar's hairline) — it triggered neither, so
R17's signed height stands unshrunk. Shrinking `h27` was never on the table: the operator questioned
the **offset**, not the size.

**Two things this decision leaves open rather than settling.**

1. **The bar's own type is not on one line with itself.** `.utility`'s 「로그인」 is centred in the full
   51px box while `.link` stretches with a 2px transparent bottom border (a 49px cell), so the account
   slot's Hangul ink bottom is **31.75** against the links' **30.875–31.125**. Pre-existing since
   R2/R8, untouched by R17, R18 and this change. Because the mark is now aligned to the **links** —
   the operator's own stated reference — it sits **0.75px above** 로그인 where under R17 it sat 0.25px
   below. Aligning to the links **exposed** a 0.6–0.75px discrepancy; it did not create one. Whether
   `.utility` moves onto the links' line is an **operator decision, open**.
2. **The footer's painted residual is not a constant.** Chrome pixel-snaps the mark's paint origin to
   whole CSS pixels when its layout y is fractional, and the footer's y varies by route, so the
   painted residual moves by up to ±0.5px with page position. No single offset removes it, and a
   `getBoundingClientRect()` reading and a screenshot will legitimately disagree by that much.

**The standing rule held, and this check honours it.** The guard shipped with the change
(「로고가 옆 글자와 한 줄로 읽힌다」, `qa.md`) was proved able to report failure before it was trusted:
re-painted at R17's `-7` it fails on every nav and footer surface, worst **1.125px**, in dev and in a
production build alike. That is the sixth wrong signed value in this phase and the fifth check to be
built with its own failing input — and it is a different species from the other four. **Those four
could not fail. This one could, and it passed, because it was pointed at the wrong reference.** The
lesson D-29 leaves ("a guard is only a guard if some input makes it report failure") therefore needs a
second half: *and it is only the right guard if it is pointed at what the operator is actually looking
at.*


## Superseded Decisions

- **R17's `INK_OFFSET 0.2628 × H` — the mark on the *row's* optical centre — is superseded by
  **D-30** (the operator, at P10's round-3 acceptance gate; R18 had re-confirmed it). The band now
  sits on the **neighbouring Hangul's** ink bottom: `{27: 8, 24: 6}`, `translateY(-8px)` / `-6px`.
  **The heights `h27` / `h24` are not superseded** — only the offset that placed them.**
...
- **R2 §Page shell's wordmark heights h19/h17 and its box-centred placement are superseded by R17**
  (h27 / h24, ink-aligned) — see D-28.
- **R17's wordmark trim box (1292×371), its opaque `#0a1310` favicon tile and its single 84 % ink
  width are superseded by R18** (`P10.review`, D-29): 1247×371, a transparent tile with `#2b8e6c`
  ink, and **75 % for the favicon while the launcher keeps 84 %**. R17's vertical geometry and its
  "one artwork, one rule" signature on the *crop* are **not** superseded — only the placement rules
  diverged, and only on one surface.
- **R2's `.link { display: inline-flex }` — and with it the assumption that an active nav item may
  change width — is superseded by R18 §②/§②b.** The active state is still 600 + a 2px white
  underline; what changed is that the cell now reserves the 600 width whether or not the item is
  active, on the nav and on `/ops`'s tab row. The ≤480 sheet and the landing board are deliberately
  outside the rule.
- **R6's launcher mark and the operator note granting it the product's one ambient-motion exception
  are superseded by R17.** The 22×22 Saturn, its 4.5s band, its 14s two-half ring drift and the
  exception itself are all gone; hover response is colour. What is **not** superseded: R2's cosmos
  backdrop and the streaming caret still move, so the claim is "the launcher's exception expired",
  never "the product has no ambient motion".
- **The assets README's "no favicon, and this mark does not become one" section is superseded by
  R17** — 16/32/180 on an opaque tile, from an operator-supplied square symbol export.
- **R1's vendored `foundations/fonts.css` and its 2,057,688-byte `PretendardVariable.woff2` are
  superseded by operator instruction (P10.S7)**, not by an agent's judgement: both class-A exports
  marked *"do not edit"* are deleted, their sha256s recorded in `frontend/public/assets/README.md`,
  and the type is now a self-hosted subset. `foundations/tokens.css` is **not** superseded and was
  not touched (R17 `Token delta: None`); the two font tokens are overridden in application code.
- **R7's "an identifier, so it stays raw and mono" for the `/ops` mark is superseded by the
  operator's gate answer** (*"drop mono, use Pretendard 600"*, landing on Noto Sans KR 600). The rule
  was written for a latin string; against `주주의관제탑 운영` it styled exactly **one glyph of nine** —
  the space — which is what produced the fake double space. The string itself is unchanged, and all
  nine glyphs now paint in one face.
