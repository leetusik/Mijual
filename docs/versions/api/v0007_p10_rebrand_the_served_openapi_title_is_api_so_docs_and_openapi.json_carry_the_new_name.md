...
doc_id: api
version: v0007
created_at: 2026-08-30T03:15:10+09:00
source: P10.REVIEW
summary: P10 rebrand — the served OpenAPI title is 주주의관제탑 API, so /docs and /openapi.json carry the new name
previous: v0006_p9_smart_assistant_the_r16_sse_vocabulary_block_id_persistent_with_in-place_replacement_the_transient_status_frame_data_and_calc_blocks_text.unverified_spans_done.filings_and_the_footer_links_suppression_rules
---
...
- **The served OpenAPI title is the product's name, and it is an outward surface** (P10). `TITLE` in
  `src/mijual/web/app.py` reads **`주주의관제탑 API`**, so `GET /openapi.json` and the Swagger page at
  `GET /docs` both carry it — verified served, not merely edited. The module path `mijual.web.app`
  and the `X-Mijual-CSRF` header keep the old spelling on purpose: they are identifiers, and P10's
  rename was scoped to what a user can read.
