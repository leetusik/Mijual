...
doc_id: backend
version: v0008
created_at: 2026-08-31T22:57:47+09:00
source: P11.REVIEW
summary: P11 fixes: two new read surfaces — GET /ask/start-cards picks the start screen's companies from the live corpus per request, and GET /site/contact serves the operator's one contact string split into its parts
previous: v0007_p10_rebrand_the_agent_s_system_instruction_and_tool_descriptions_name_the_retired_latin_gloss_is_dropped_rather_than_romanized
---
...
- **`mijual.agent`** — P6's agent package, the only place in the system that reaches a model from a
  ...
  - `instructions.py` · `copy.py` — the system instruction, and the signed Korean strings with their
    ...
    **The prompt is a user-facing surface, because it decides the name the model *says*** (P10). It
    spelled the product **미주얼** — a *third* spelling that a `미주알` grep never sees, so the product
    had been shipping two spellings of its own name. **13 live strings** now read 주주의관제탑: nine in
    `instructions.py` (`_ROLE` opens "You are 주주의관제탑's 해설 agent"), `tools.py`'s `DATA_BOUNDARY`,
    and three in `declarations.py`. **The retired latin mark is reworded, never substituted**, because
    there is no romanized replacement: `instructions.py`'s `미주얼(Mijual)` gloss is **dropped
    outright**, and `save_feedback`'s description, where *Mijual* was an English noun, now says "this
    product itself". Only names changed — no sentence was reflowed, reordered or rewritten, which
    matters because the instruction is a static cache prefix (see *Architecture*). Code comments keep
    미주얼 as history.
...
- **`mijual.web.site` + `routers/site.py`** — the site-config surface (P11.F2). **`GET
  /site/contact`** answers `{contact, email, phone}` from the single free-text
  `Settings.operator_contact`, three `null`s and a **200** when it is unset — never an error, because
  the global footer calls it on every page. `split_contact()` does the splitting **on the server,
  once**, so the agent's answer and the chrome's row cannot drift, and it is tolerant of how the
  operator writes the value (labels, order and hyphens are theirs; `01037729916, name@example.com`
  splits correctly). The setting stays **one string, one setting** — `get_contact` is unchanged and
  no `require_` accessor was added, so nothing can fail for want of it. Its own module rather than a
  corner of `routers/ask.py`: this is site-wide config with two unrelated consumers.
...
- **`mijual.web.reads`** — the batched read layer (`load_board` · `load_summary` ·
  ...
  - **`load_start_cards(session, today=)`** (**P11.F1**) — the `/ask` start screen's two 공시
    companies, picked **per request** so a start card can never become a question the product can no
    longer answer. It ranks over `_board_views` — the board's own reading of the corpus, gated by the
    persisted verdict *and* the derived contract like every other surface, so no new query layer
    exists. The 검색 slot takes an issuer whose exposable filings are one 권리 가족 (so the card's own
    건수 and the tool row's count the same thing), multi-hit first, then most filings, then
    `corp_code`; the 계산 slot an exposable ① still exposing an `allotment_ratio` with a **future**
    deadline, D-20…D-60 first, single-① issuers preferred so the read cannot land on a sibling
    filing, then most headroom, then `rcept_no`; and the two slots avoid each other's issuer where
    the corpus allows. **Both names are re-checked through the agent's own `find_corps`** (not
    `resolve_corp` — the card's sentence travels to `search_events`, whose contract is a list) and a
    candidate reaching two issuers is rejected, bounded to the first 8 ranked candidates per slot.
    An empty corpus answers `null` per slot with a **200**, which the frontend renders as its static
    fallback card. `GET /ask/start-cards` (`routers/ask.py`) is the thin route over it, and it
    carries **no Korean** — the sentences are the frontend's.
