...
doc_id: experience
version: v0011
created_at: 2026-08-31T22:56:14+09:00
source: P11.REVIEW
summary: P11 fixes: the ask start screen offers four cards whose two 공시 companies the server picks from the live corpus per request, and the 연락처 card answers the operator's configured contact
previous: v0010_p11_the_ask_start_screen_offers_six_cards_one_per_agent_capability_the_seven-tool_list_and_the_never-calculates_line_are_corrected
---
...
- **The 운영자 연락처 is configured, and it is published in two places** (P11.F2, at the operator's
  explicit instruction). `get_contact` answers with the operator's own email and phone instead of the
  honest-unset line, and **the same two values sit in the footer of every reader page** (email in the
  UI face, phone in mono, joined to the existing 자료/© row). The honest-unset behaviour is not
  retired — it is what an **unset** `MIJUAL_OPERATOR_CONTACT` still produces, in the agent's voice
  only: the chrome then renders no contact line at all rather than an empty label. `/ops` renders no
  footer, so the values appear on no operator page.
...
- **The start screen offers four question cards, and the two that name a company are resolved from
  the live corpus at request time** (P11 as re-cut by `P11.F1` on the operator's gate report;
  supersedes both the six-card set this section carried between P11.S2 and that report, and R16
  D11's 「4장」-as-signed-strings). Pressing them one at a time still demonstrates the product's
  capabilities:
  - `search_events` — 「{회사} 전환사채 공시가 몇 건이나 있나요?」, the 회사 being **whoever the server
    picked when the page loaded**;
  - `calculate` — 「{회사} 유상증자, 1,000주 보유 시 배정 신주는 몇 주인가요?」, again a served 회사; the
    1,000주 is the reader's own input and shows as 「입력」, and the turn walks 검색 → **이벤트 읽기**
    → 계산, so `get_event` is demonstrated **inside this chain** rather than by a card of its own;
  - `get_portfolio` — 샘플 포트폴리오, answered 「구성 예시」 (static sentence, no company);
  - `get_contact` — 「운영자에게 직접 연락하려면 어디로 하면 되나요?」 (static sentence).

  **Two capabilities are deliberately not on the start screen.** `security_check` never was (showing
  it off would invite an injection attempt from the first screen). **`save_feedback` is no longer
  there**: the operator dropped the 의견 card at the P11 acceptance gate, accepting that the
  capability goes undemonstrated there — so *pressing a start card no longer writes anything*, and
  the 운영자 검토 대기열 gains no row from a demo walk.

  **Why the companies are served, not typed.** A card naming a fixed company becomes a question the
  product can no longer answer as its filing ages — three of R16's original four had already gone
  dead that way. The two 공시 slots are now picked per request from the board's own reading of the
  corpus (`GET /ask/start-cards`; the 검색 slot takes an issuer with several exposable ② filings, the
  계산 slot an ① that still exposes a 신주배정비율 before its deadline, and both names must resolve to
  exactly one issuer through the agent's own search). Korean never travels on that wire —
  `components/ask/copy.ts` stays the single home of every Korean string and holds the two templates
  plus a **static fallback** for when the API cannot answer. The card's sentence **is** the question
  sent, verbatim, exactly as before.
