...
source: P8.REVIEW
summary: P8 design polish pass — the vocky env row covers read and capture; the 의견 path writes no row on this side
previous: v0005_p6_apply_phase_the_two_anonymous_conversation_tables_no_account_email_ip_ua_column_no_foreign_key_in_either_direction_a_randomly_minted_session_handle_and_rows_that_are_not_re-collectable
---
...
**P8 (디자인 폴리시 패스) changed no stored data at all.** It added one outward path — the reader's
의견 send — and that path is write-through-to-vocky only: no table, no column, no row. The only
data-shaped addition in the phase is a derived field on an existing read (`next_lapse.tie_count` on
`GET /board/summary`, computed from the ordered `lapse_pending` list already served — see `api`).
...
| `MIJUAL_VOCKY_API_BASE` / `_API_KEY` | P4 | the vocky observation read **and** (P8) the reader's 의견 capture | `vk_` key, masked repr, raises only on use, never logged, never in a URL, **https matters**. The capture path (`POST /feedback`) **writes no row on this side** — no schema change, no new table, and the reader's message is not persisted here |
