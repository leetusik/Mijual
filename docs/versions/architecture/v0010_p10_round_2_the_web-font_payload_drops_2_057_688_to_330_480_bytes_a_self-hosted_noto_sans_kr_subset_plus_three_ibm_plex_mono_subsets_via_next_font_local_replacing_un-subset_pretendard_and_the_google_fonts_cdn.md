...
doc_id: architecture
version: v0010
created_at: 2026-08-31T04:30:43+09:00
source: P10.REVIEW
summary: P10 round 2 — the web-font payload drops 2,057,688 to 330,480 bytes: a self-hosted Noto Sans KR subset plus three IBM Plex Mono subsets via next/font/local, replacing un-subset Pretendard and the Google Fonts CDN
previous: v0009_p10_rebrand_renaming_the_product_inside_the_system_instruction_invalidates_the_static_cache_prefix_once_and_that_is_accepted
---
...
| persistence | **Postgres** via **SQLAlchemy 2 + psycopg3** | local docker, host port **5434** (moved off 5433, which another project on the operator's machine holds) |
...
| web fonts | **self-hosted subsets via `next/font/local`** (P10.S7, adopting `changple_web`'s pipeline): Noto Sans KR `wght 100–900` + IBM Plex Mono 400/500/600 | **330,480 B total**, down from **2,057,688 B** — and **no third-party origin** remains |
...
- `frontend/` — the Next.js app (`app/` routes · `components/` · `lib/` · `app/fonts/` with the
  self-hosted subsets and both OFL files · `public/foundations/` with the byte-verbatim vendored
  `tokens.css` · `public/assets/` with the delivered binary design assets and their derivations ·
  `scripts/` with the font regeneration tools)
...
- **The system instruction is a static cache prefix, and that is a standing constraint.**
  ...
  question rather than a claim that caching never happens. **Editing the prefix invalidates it once, by
  construction, and P10 did exactly that**: renaming the product inside `_ROLE`, `DATA_BOUNDARY` and
  the tool declarations changes the prefix bytes, so the first turn after the rename re-primes the
  cache. A one-time invalidation is the accepted price of any prompt edit — what the constraint
  forbids is a *per-turn* value above the seam, and the rename introduced none: only names changed,
  and nothing was reflowed or reordered.
...
**Self-hosted, subset, and no third-party origin** (P10.S7, at the operator's instruction to follow
their own `changple_web` implementation).

| ships | bytes |
|---|---|
| `app/fonts/NotoSansKR.subset.woff2` (variable `wght 100–900`) | 291,072 |
| `app/fonts/IBMPlexMono-{Regular,Medium,SemiBold}.subset.woff2` | 12,712 + 13,012 + 13,684 |
| **total** | **330,480** |

Retired with it: `public/assets/fonts/PretendardVariable.woff2` (**2,057,688 B**, un-subset) and
`public/foundations/fonts.css`, whose `@import` pulled IBM Plex Mono from the **Google Fonts CDN**.
Both were class-A design-project exports; their sha256s are recorded in
`frontend/public/assets/README.md`. Net **6.2×** smaller, and — measured across five routes in dev
and in a production build — **zero third-party requests** remain (see `security`).

Delivery is `next/font/local` with `display: "swap"`, `preload: true` for the Korean face and
`false` for the mono, exposed as CSS variables with `system-ui / -apple-system / Apple SD Gothic
Neo / Malgun Gothic` fallbacks. Exactly one `<link rel=preload as=font>` is emitted, for
NotoSansKR — in production Next emits it as an RSC resource hint that React materialises, so it is
absent from the HTML string and present in the live DOM.

**Two constraints carried over deliberately, because the reference implementation already paid for
both.** The charset is **auto-extracted** by `scripts/gen-korean-charset.mjs` from everything the app
renders (509 unique Korean chars over 142 files under `app/`, `components/`, `lib/`) — a
hand-maintained list goes stale and renders missing syllables **자모 분리**. And the subset
**omits the conjoining-jamo block U+1100–11FF**, so an unknown syllable falls back to the system font
*composed* rather than decomposed. `scripts/subset_noto_sans_kr.sh` re-subsets from a **pinned**
google/fonts commit (`4efc2774…`) with `pyftsubset` + brotli; the scripts are dev-only.

**One adaptation away from the reference, because Mijual's layout differs.** `changple_web` subsets
to *rendered* glyphs only and lets dynamic DB copy fall back to the OS face; this product renders
**dynamic DART company names** on nearly every surface, so the subset is widened to the **KS X 1001
wansung set (2,350 syllables)**. Measured, same source and ranges: app glyphs alone **94,604 B**
(fails — company names fall back), **+ KS X 1001 291,072 B (adopted)**, + the full 11,172-syllable
block **1,022,828 B**. It is the smallest that works: across **all 654 distinct Hangul syllables in
the corpus** (614 company names, agent answers, extraction summaries, quoted filings) KS X 1001
misses exactly **one** — `쳥`, a typo inside a real filing — and covers all 360 company-name
syllables. Instancing the weight axis is *not* a cheaper lever (it makes option (c) slightly
*larger*, 1,053,156 B, because the instancer bakes deltas), so the full axis is kept. **Widening to
full coverage costs one variable** (`HANGUL_COVERAGE=full`) and 732 KB.

Verified in a real browser rather than asserted: `CSS.getPlatformFontsForNode` reports a live DART
company name (`HLB제약`) and the UI beside it painted by the **same** shipped face
(`isCustomFont: true`, 5/5 glyphs), and no Pretendard face is present in `document.fonts` at all.
