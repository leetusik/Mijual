...
doc_id: product
version: v0009
created_at: 2026-08-30T03:12:00+09:00
source: P10.REVIEW
summary: P10 rebrand — the product is 주주의관제탑; the symbol-mark / favicon gap R2 closed is open again
previous: v0008_p8_design_polish_pass_the_layer_is_not_the_board_is_15_rows_and_keeps_itself_current_the_last_english_screen_is_gone_portfolio_answers_anonymously_with_the_sample_and_481-767_gets_the_full-width_ai_page
---
...
- **There is still no favicon-scale symbol mark, and P10 re-opened the gap.** R1 disclosed it, R2
  closed it with the ring logo, and P10 retired those assets: the operator's mark is a Korean
  wordmark with a small sparkle cluster and **no ring**, natural 1213×319, and it does not reduce
  legibly to 32px (measured at P10.S2 — at that size it is mush). Nothing was substituted, generated
  or placeheld, so the product ships **no `favicon.ico`, no `app/icon.*`, no `manifest` and no
  `icons` key** — verified absent in dev and in a production build. The operator chooses: ship no
  favicon as today, supply a square symbol export, or authorize cropping the sparkle cluster as a
  symbol. Only the operator can pick, because the assets README forbids a slice inventing one.
