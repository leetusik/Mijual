...
doc_id: product
version: v0005
created_at: 2026-08-22T22:54:42+09:00
source: P6.REVIEW
summary: P6 apply phase: the AI 질문 agent exists — an autonomous citation-forced agent on every surface, unlimited and anonymous, refusing rather than guessing; the whole designed product is now built
previous: v0004_p5_apply_phase_the_designed_product_is_built_the_surfaces_exist_the_notification_boundary_and_what_the_build_closed
---
...
- **Two more agent-surface calls left to the operator**, each honest as shipped: a completed refusal
  currently carries a footer (`근거 0건 · 시각`), which the record neither signs nor forbids; and the
  agent prints numbers exactly as the contract hands them over (`3200원`), while every other surface
  in the product formats them (`3,200원`) — formatting inside prose would be the agent transforming a
