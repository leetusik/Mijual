...
doc_id: product
version: v0010
created_at: 2026-08-31T04:35:03+09:00
source: P10.REVIEW
summary: P10 round 2 — the favicon-scale symbol gap open since R1 is closed: the operator supplied a square symbol export, so the mark ships at 16/32/180 with nothing cropped or invented
previous: v0009_p10_rebrand_the_product_is_the_symbol-mark_favicon_gap_r2_closed_is_open_again
---
...
- ~~**There is still no favicon-scale symbol mark**~~ — **closed 2026-08-31 (P10 round 2)**, the
  ...
  **satisfied, not relaxed**. The sparkle cluster is now a **first-class symbol mark** — painted in
  the UI by CSS `mask` + `currentColor` from one asset, and composited onto an opaque `#0a1310` tile
  at **16 / 32 / 180** for the favicon and the apple-touch icon. Verified served in dev and in a
  production build, on every reader and `/ops` route. **One limitation is disclosed rather than
  fixed:** at 16px the five small dots are ~1.4px each and read soft; the fix (a second crop showing
  one star) was explicitly *not* adopted, because one artwork with one rule beats two that disagree.
