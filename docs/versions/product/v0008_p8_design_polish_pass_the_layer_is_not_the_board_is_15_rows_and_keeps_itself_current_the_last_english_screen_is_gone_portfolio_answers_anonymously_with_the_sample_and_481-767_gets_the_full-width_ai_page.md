...
source: P8.REVIEW
summary: P8 design polish pass — the layer is 보유 종목 not 포트폴리오, the board is 15 rows and keeps itself current, the last English screen is gone, /portfolio answers anonymously with the sample, and 481-767 gets the full-width AI 질문 page
previous: v0007_p7_fix_pass_the_nav_is_two_slots_lives_on_the_landing_hero_the_board_is_read_30_rows_at_a_time_and_the_search_offers_candidates_while_the_reader_types_without_the_resolver_ever_guessing
---
...
- **The last English screen a Korean-only reader could reach is gone.** A non-exposable filing, a
  mistyped address and an unmatched path all render 미주알's own Korean not-found, which **still says
...
- **계정 삭제 states its consequence only while armed**, and 미주알 owns its 의견 screen — a reader's
