...
doc_id: operations
version: v0005
created_at: 2026-08-22T18:12:00+09:00
source: P5.REVIEW
summary: P5 apply phase: the six-stage beat with the run log, running the API and frontend locally, the re-derivation order, and the operator console as built
previous: v0004_p3_operations_the_design_grounding-pack_export_command_at_zero_spend_and_the_observation-only_operating_rules_the_signed_admin_panel_imposes
---
...
processes an operator now runs locally (uvicorn + the Next app). Deployment stays **P4**'s.
...
- **The three P6-facing tabs serve honest zeros.** 대화 로그 / 익명 세션 / the feedback queue read a
  storage-agnostic port that P5 implements as an empty source, so they render **0건** rather than an
  invented 「준비 중」 string. P6 implements the port and changes no route.
